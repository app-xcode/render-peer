...
<body class="bg-gray-50 relative overflow-hidden">
    ...
    <script src="new-assets/js/peerjs.min.js"></script>
    <script>
        const chat_key = 'kon1';
        ...
        function initPeer() {
            let config = true ? {
                host: 'render-peer.up.railway.app',
                port: 443,
                path: "/peerjs/myapp",
                secure: true,
                debug: 2,
            ...
            peer = new Peer(sender_type + sender_id, config);

            peer.on('open', id => {
                console.log('✅ Peer online as', id);
                $('#peerStatus').text(`Online`);
                connectToPeer();
            });
        ...
        // -------------------------
        // Setup koneksi antar Peer
        // -------------------------
        function connectToPeer() {
            console.log('🔗 Connecting to', target_type + target_id);
            $('#peerStatus').text(`Connecting`);
            conn = peer.connect(target_type + target_id);
            setupConnection(conn);
        }
        ...
        function setupConnection(connection) {
            ...
            conn.on('open', async () => {
                ...
                $('#peerStatus').text(`Terhubung`);
                //kirim pesan yang belum terkirim
                conn.send('connect');
            });
        ...
        function reconnect() {
            setTimeout(() => {
                if (!conn || !conn.open) connectToPeer();
            }, 2000);
        }
